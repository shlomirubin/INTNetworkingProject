...
if [ $# -ge 2 ]; then
  if [ $# -eq 2 ]; then
    ssh -t -i "$KEY_PATH" ubuntu@"$PUBLIC_IP" "ssh -t -i /home/ubuntu/.ssh/RotatedKey" ubuntu@"$PRIVATE_IP"
  else
    # Case 3: Run a command on the private instance
    shift 2
    ssh -i "$KEY_PATH" ubuntu@"$PUBLIC_IP" "ssh -i /home/ubuntu/.ssh/RotatedKey" ubuntu@"$PRIVATE_IP" "$@"
  fi
  exit $?
fi
